...
1. Replace local ad-hoc buttons (`ToolButton`, `GameButton`, unexported components) with shared `Button` / `IconButton`.
2. Replace local modals and headers with shared `Modal` and `StatsHeader`.
3. Replace hardcoded hex colors with semantic CSS custom properties (`--bg`, `--surface`, `--border`, `--text`, `--text-muted`, `--accent`).
4. Wire `isEink` prop to eliminate drop shadows, blur filters, and transitions.
